...
export function ReviewEditor({ selectedNFT, reviewText, setReviewText, loading, onSubmit, submitState, cid }: {
    selectedNFT: any;
    reviewText: string;
    ...
    return (
      ...
      />
            <button
                className={`mx-auto mt-[20px] w-[30%] py-[10px] rounded-[24px] font-[600] text-[18px] shadow-neon-cyan transition-all duration-300 neon-focus flex items-center justify-center gap-2
          ${selectedNFT && reviewText
                ...
                onClick={onSubmit}
            >
                <Upload className="w-5 h-5 mr-[5px]" />
                {loading ? "Uploading..." : "Publish review"}
            </button>
            {submitState === "success" && (
                <div className="mt-[30px] flex items-center justify-center gap-2 mt-4 text-[green] text-[18px] font-bold animate-bounce">
